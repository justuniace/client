...
function Upload({ updateTotalStudentsCount }) {
  ...
  const [yearStartedValue, setYearStartedValue] = useState(0);
  ...
  const handleAdditionalInsertClick = async () => {
    try {
      ...
      console.log("Faculty Programs Table Records:", programsData);
      ...
      data.forEach((row) => {
        ...
        if (
          ...
          row.some((cell) =>
            cell
              .toUpperCase()
              .includes("BACHELOR OF SCIENCE IN CIVIL ENGINEERING")
          )
        ) {
          course_code = "BSCE";
        }
        ...
        if (row.some((cell) => cell.toUpperCase().includes("FIRST SEMESTER"))) {
          ...
          row.some((cell) => cell.toUpperCase().includes("BRIDGING"))
        ) {
          currentSemester = "BRIDGING";
        }
        ...
        if (
          ...
          !row
            .map((cell) => cell.trim())
            .includes("DIPLOMA IN INFORMATION TECHNOLOGY (DIT)") &&
          !row
            .map((cell) => cell.trim())
          ...
          !row.map((cell) => cell.trim()).includes("SUMMER SEMESTER") &&
          !row.map((cell) => cell.trim()).includes("BRIDGING") &&
          ...
          if (!matchingProgramFound) {
            ...
            if (matchingProgram) {
              ...
              const { program_id, program_abbr, program_name } =
                matchingProgram;
              if (program_id === (facultyProgram)) {
                console.log("Program ID matches with the course code");
                matchingProgramFound = true;
              } else {
                console.log(program_id, facultyProgram);
                console.log(
                  "Selected program does not match with the selected file"
                );
      ...
      if (!matchingProgramFound) {
        ...
        // );
        // const studentCount = studentCountResponse.data.count;

        // // Now you can use the studentCount however you want, such as displaying it in an alert
        // console.log("Student Count:", studentCount);
        // updateTotalStudentsCount(studentCount.length);
      }
...
const handleYearStartedChange = (e) => {
  ...
  const intValue = validatedValue !== "" ? parseInt(validatedValue) : null; // Convert to integer or null
  setYearStartedValue(intValue);
};
