...
function HistoryAll({
  ...
  const [year5Data, setYear5Data] = useState([]);
  const [year6Data, setYear6Data] = useState([]);
    ...
    const fetchData = async () => {
      try {
        ...
        if (studentData) {
          ...
          console.log("program", studentData.program_id);
        } else {
  ...
  useEffect(() => {
    axios
      .get(`${endPoint}/evaluate-student?studentNumber=${studentNumber}`)
      .then((response) => {
        ...
        const year5 = response.data.filter((item) => item.eval_year === 5);
        const year6 = response.data.filter((item) => item.eval_year === 6);
        ...
        const bridging = response.data.filter(
          (item) => item.eval_sem === "BRIDGING"
        );
  ...
  useEffect(() => {
    ...
    console.log(" year5Data", year5Data);
    console.log(" year5Data", year6Data);

    console.log("First sem", firstsem);
    console.log("Second sem", secondsem);
    console.log("Summer sem", summersem);
    console.log("Bridging sem", bridging);
  }, [
    bridging,
    curriculumData,
    evaluationData,
