...
function UsersEvaluation({ studentNumber, evalYearValue, evalSemValue }) {
  ...
  console.log("Student number in user", studentNumber);
  ...
  useEffect(() => {
    const yearstarted = getCourseType(studentNumber);

    const fetchCousePrerequisiteData = async () => {
      console.log("pog", programId);
      console.log("current in", yearstarted);

      try {
        const response = await fetch(
          `${endPoint}/curriculum-prerequisite?programId=${programId}&yearStarted=${yearstarted}`
        );
        ...
        const capitalizedData = data.map((course) => ({
          ...course,
          course_sem: course.course_sem,
        }));

        console.log(
          "Fetched course code of prerequisite data:",
          capitalizedData
        );

        // Set the fetched data in the local state
        setCourseCodePrerequisite(capitalizedData);
        console.log("Prelogs", capitalizedData);
      } catch (error) {
  ...
  console.log("j", courseCodePrerequisite);
  ...
  useEffect(() => {
    ...
    const fetchValidatedCoursesDetails = async () => {
      try {
        const validatedCoursesDetailsPromises = validatedCourseCode.map(
        ...
        // Fetch details for validated courses
        const detailsResponses = await Promise.all(
          validatedCoursesDetailsPromises
        );
        ...
        console.log("Validated course details:", validatedCoursesDetails);

        const matchedCourses = [];
        gradesResponse.data.forEach((gradeItem) => {
          validatedCoursesDetails.forEach((courseDetail) => {
            if (gradeItem.course_id === courseDetail.course_id) {
              matchedCourses.push({
        ...
        // Set the matched courses in local state
        setGradesCourseId(matchedCourses);
        console.log("setGradesCourseId", matchedCourses);
      } catch (error) {
        console.error("Error fetching course details:", error);
      }
    };

    // Call the async function to fetch validated course details
    fetchValidatedCoursesDetails();
  }, [validatedCourseCode]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (evalYearValue === "1" && evalSemValue === "FIRST SEMESTER") {
        endpoint = `${endPoint}/curriculum-first-first?programId=${programId}&year_started=${currentCourseType}`;
      } else if (evalYearValue === "1" && evalSemValue === "SECOND SEMESTER") {
        endpoint = `${endPoint}/curriculumfirst-second?programId=${programId}&year_started=${currentCourseType}`;
      } else if (evalYearValue === "2" && evalSemValue === "FIRST SEMESTER") {
        endpoint = `${endPoint}/curriculumsecond-first?programId=${programId}&year_started=${currentCourseType}`;
      } else if (evalYearValue === "2" && evalSemValue === "SECOND SEMESTER") {
        endpoint = `${endPoint}/curriculumsecond-second?programId=${programId}&year_started=${currentCourseType}`;
      } else if (evalYearValue === "3" && evalSemValue === "FIRST SEMESTER") {
        endpoint = `${endPoint}/curriculumthird-first?programId=${programId}&year_started=${currentCourseType}`;
      } else if (evalYearValue === "3" && evalSemValue === "SECOND SEMESTER") {
        endpoint = `${endPoint}/curriculumthird-second?programId=${programId}&year_started=${currentCourseType}`;
      } else if (evalYearValue === "4" && evalSemValue === "FIRST SEMESTER") {
        endpoint = `${endPoint}/curriculumfourth-first?programId=${programId}&year_started=${currentCourseType}`;
      } else if (evalYearValue === "4" && evalSemValue === "SECOND SEMESTER") {
        endpoint = `${endPoint}/curriculumfourth-second?programId=${programId}&year_started=${currentCourseType}`;
      } else if (evalSemValue.toUpperCase() === "SUMMER SEMESTER") {
        endpoint = `${endPoint}/curriculumsummer?programId=${programId}&year_started=${currentCourseType}`;
  ...
  const filterCourses = (year, semester) => {
    ...
    return courses.filter((courseItem) => {
      ...
      //validatedCourseID
      // console.log("coursepre", courseCodePrereq);
      //console.log("coursepre2", courseCodePrerequisite);
      ...
      if (shouldIncludeCourse) {
        ...
        // console.log(
        //   `Excluded Course: ${courseItem.course_code}, ID: ${courseItem.course_id}, Year: ${courseItem.course_year}, Semester: ${courseItem.course_sem} prerequisite: ${courseItem.pre_requisite}`
        // );
  ...
  useEffect(() => {
    ...
    const fetchTotalEvaluatedCreditUnits = async () => {
      try {
        ...
        const totalEvaluatedCreditUnits =
          response.data.totalEvaluatedCreditUnits;
  ...
  const handleRecommendAll = async () => {
    try {
      ...
      const coursesToRecommend = uniqueDisplayedCourseCodes.filter((code) => {
        const hasGrade = evaluatedCourses.some(
          (course) =>
            course.course_reco.toUpperCase().replace(/\s/g, "") ===
              code.toUpperCase().replace(/\s/g, "") &&
            course.student_number === studentNumber &&
            course.grades !== null // Check if the course has a grade
        );
        const hasGradesInIds = gradesCourseId.some(
          (course) =>
            course.course_code.toUpperCase().replace(/\s/g, "") ===
              code.toUpperCase().replace(/\s/g, "") && course.grades !== null // Check if the course has grades
        );
        return !hasGrade && !hasGradesInIds;
      });
  ...
  return (
    <Box w="100%" position="relative">
      {showAnalytics ? (
        ...
        <Flex mt="3rem" overflow="visible" flexDirection="column" w="100%">
          ...
          <Box
            ...
            borderRadius="md"
            position="sticky"
            top="6rem" // Adjust this value according to your navbar height
            zIndex="sticky"
          ...
          >
            ...
            <HStack mt="1rem" justifyContent="space-between">
              ...
              <HStack>
                ...
                <Text>
                  {totalCreditUnits - totalRecommendedCreditUnits < 0
                    ? 0
                    : totalCreditUnits - totalRecommendedCreditUnits}{" "}
                  Unit(s)
                </Text>
              </HStack>
            </HStack>
          ...
          <HStack mt="1rem" ml={allowOverload ? "42rem" : "52rem"}>
            {((programName.startsWith("B") &&
              evalYearValue >= 4 &&
              evalYearValue <= 6) ||
              (programName.startsWith("D") &&
                evalYearValue >= 3 &&
                evalYearValue <= 5)) && (
              <Button
                bg="#740202"
          ...
          <Wrap>
            {Object.keys(filteredCourses).map((key) => {
              ...
              const schoolYear = `${schoolYearStart}-${currentYear}`;
              ...
              if (filteredCourseItems.length > 0) {
                ...
                return (
                  <div key={key}>
                    <VStack mt="3rem" align="flex-start">
                      <HStack>
                        <Text fontSize="15px" fontWeight="semibold">
                          Year Level:
                        </Text>
                        <Text w="10rem" fontWeight="md" fontSize="15px">
                      ...
                      <HStack spacing="31rem" justifyContent="space-between">
                        <HStack>
                          ...
                          <Text w="20rem" fontWeight="md" fontSize="15px">
                      ...
                      >
                        ...
                        <Tbody>
                          {filteredCourseItems.map((courseItem) => {
                            ...
                            const matchedCourse = gradesCourseId.find(
                              (item) => item.course_id === courseItem.course_id
                            );

                            const hasGrade =
                              matchedCourse && matchedCourse.grades;

                            const excludeFromRecommendations = hasGrade;

                            return (
                              <Tr
                                key={courseItem.course_code}
                                style={{
                              ...
                              >
                                ...
                                <Td>
                                  {hasGrade
                                    ? matchedCourse.grades.toFixed(2)
                                    : ""}
                                </Td>
                            ...
                            >
                              {filteredCourseItems
                                .filter((courseItem) => {
                                  const matchedCourse = gradesCourseId.find(
                                    (item) =>
                                      item.course_id === courseItem.course_id
                                  );
                                  return (
                                    !matchedCourse || !matchedCourse.grades
                                  );
...
UsersEvaluation.propTypes = {
  ...
  evalYearValue: PropTypes.number.isRequired,
