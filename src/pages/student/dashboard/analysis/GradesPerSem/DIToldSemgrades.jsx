...
export default function DIToldGradesperSemester({
  studentNumber,
  onGradesDataChange,
}) {
  ...
  const [firstYearSummerSem, setFirstYearSummerSem] = useState([]);
  ...
  const [firstYearSummerSemAverage, setFirstYearSummerSemAverage] = useState(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //First Year Summer Sem
        const firstYearSummerSemData =
          groupedData["SUMMER SEMESTER_1"]?.data || [];

        const filteredFirstYearSummerSemData = firstYearSummerSemData.filter(
          (grade) => grade.grades > 0
        );
        setFirstYearSummerSem(filteredFirstYearSummerSemData);

        //compute
        const sumGradesFirstSumSem = filteredFirstYearSummerSemData.reduce(
          (sum, grade) => sum + grade.grades,
          0
        );

        const averageGradeSummerFirstSem =
          filteredSecondYearFirstSemData.length > 0
            ? sumGradesFirstSumSem / filteredFirstYearSummerSemData.length
            : 0;

        // Check if average is NaN (optional)
        if (!isNaN(averageGradeSummerFirstSem)) {
          const roundedAverage = parseFloat(
            averageGradeSummerFirstSem.toFixed(2)
          );
          setFirstYearSummerSemAverage(roundedAverage);
        }
        ...
        setSecondYearSecondSem(filteredSecondYearSummerSemData);

        //compute
        const sumGradesSummerSecondSem = filteredSecondYearSummerSemData.reduce(
          (sum, grade) => sum + grade.grades,
          0
        );
        ...
        const averageGradeSecondSummerSem =
          filteredSecondYearSummerSemData.length > 0
            ? sumGradesSummerSecondSem / filteredSecondYearSummerSemData.length
            : 0;
...
DIToldGradesperSemester.propTypes = {
