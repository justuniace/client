...
function DITGradesAverageLine({ studentNumber }) {
  ...
  const [averageGradesFor1SummerSemester, setAverageGradesFor1SummerSemester] =
    useState(0);
  const [averageGradesFor2SummerSemester, setAverageGradesFor2SummerSemester] =
    useState(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //1stYear summer sem
        const filteredYear1SummerGrades = mappedGradesData.filter(
          (entry) =>
            entry.course_year === 1 && entry.course_sem === "SUMMER SEMESTER"
        );

        const sumGradesYear1Summer = filteredYear1SummerGrades.reduce(
          (sum, entry) => sum + entry.grades,
          0
        );

        const countYear1GradesSummer = filteredYear1SummerGrades.length;

        const averageGradesYear1Summer =
          countYear1GradesSummer > 0
            ? (sumGradesYear1Summer / countYear1GradesSummer).toFixed(2)
            : 0;

        // Set the result in local state
        setAverageGradesFor1SummerSemester(averageGradesYear1Summer);
        console.log("Average Grades Year 1 SUmmer:", averageGradesYear1Summer);
        ...
        // Set the result in local state
        setAverageGradesFor2SummerSemester(averageGradesForSummerSemester);
        console.log(
  ...
  useEffect(() => {
    ...
    const options = {
      series: [
        {
          data: [
            averageGradesYear1,
            averageGradesFor1SummerSemester,
            averageGradesYear2,
            averageGradesFor2SummerSemester,
            averageGradesYear3,
          ],
        },
      ],
      ...
      xaxis: {
        categories: ["First", "Summer", "Second", "Summer", "Third"],
        labels: {
          style: {
            fontSize: "12px",
          },
  ...
  return (
    <VStack spacing={4} align="stretch">
      ...
      <Box id="chart" padding={{ base: "0rem", md: "2rem" }}>
        ...
        {averageGradesFor1SummerSemester !== undefined &&
          averageGradesFor1SummerSemester !== 0 && (
            <HStack>
              <Text>You have a General Weighted Average of </Text>
...
DITGradesAverageLine.propTypes = {
  studentNumber: PropTypes.string.isRequired,
};

export default DITGradesAverageLine;
